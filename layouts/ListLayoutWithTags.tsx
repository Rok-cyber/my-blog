...
import englishTagData from 'app/en/tag-data.json'
...
function Pagination({ totalPages, currentPage, locale = 'ko' }: PaginationProps) {
  ...
  return (
    <nav className="mt-10 flex items-center justify-between border-t border-gray-200 pt-7 text-sm dark:border-gray-800">
      {hasPrev ? (
        ...
        <span className="text-gray-400">← {locale === 'en' ? 'Previous' : '이전'}</span>
      )}
      ...
      {hasNext ? (
        ...
        <span className="text-gray-400">{locale === 'en' ? 'Next' : '다음'} →</span>
      )}
    </nav>
  )
}
...
export default function ListLayoutWithTags({
  ...
  const tagCounts = (isEnglish ? englishTagData : tagData) as Record<string, number>
  ...
  return (
    <div lang={isEnglish ? 'en' : 'ko'} className="py-14 sm:py-20">
      <header className="max-w-4xl">
        <p className="eyebrow">{isEnglish ? 'Biweekly AI outlook' : 'Writing archive'}</p>
        ...
        <p className="mt-6 max-w-2xl text-lg leading-8 text-gray-600 dark:text-gray-400">
          {isEnglish
            ? 'Independent analysis of AI markets, technology shifts, agent systems, and the futures they may create—published every two weeks.'
            : 'AI 시스템과 거버넌스를 중심으로, 구현을 위한 소프트웨어와 도시·공공 영역의 적용을 기록합니다.'}
        </p>
      </header>
      ...
      <div className="mt-12 grid gap-10 border-t border-gray-200 pt-8 lg:grid-cols-[220px_1fr] lg:gap-16 dark:border-gray-800">
        <aside>
          <nav
            aria-label={isEnglish ? 'Writing topics' : '글 주제'}
          ...
          >
            <Link
              ...
              className={`shrink-0 rounded-full px-3 py-1.5 text-sm font-medium transition-colors lg:rounded-md ${
                pathname.startsWith(isEnglish ? '/en/blog' : '/blog')
                  ? 'bg-primary-500 text-white'
                  : 'text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-900'
              }`}
            >
              {isEnglish ? 'All writing' : '전체 글'}{' '}
            ...
            {sortedTags.map((tag) => {
              ...
              return (
                <Link
                  key={tag}
                  href={`${isEnglish ? '/en' : ''}/tags/${slug(tag)}`}
        ...
        <div className="max-w-4xl min-w-0">
          {!displayPosts.length && (
            <p className="rounded-xl bg-gray-100 p-8 text-gray-600 dark:bg-gray-900 dark:text-gray-400">
              {isEnglish
                ? 'The first English AI outlook is coming soon. New analysis will be published every two weeks.'
                : '이 주제의 글이 아직 없습니다.'}
            </p>
          )}
